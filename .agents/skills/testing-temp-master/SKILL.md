...
The Dockerfile builds the React app and copies `switchbot-frontend/dist/` to `switchbot-backend/static/`, but locally this directory doesn't exist. Build the app first, then symlink the build output:
...
Re-run `npm run build` after any frontend change (the symlink keeps pointing at `dist/`).

For frontend-only iteration you can instead run the Vite dev server at `http://localhost:5173`:
...
- Footer: should say "Temp Master Dashboard v2.0 - Built with React + Tailwind CSS"
...
- Connection status badge shows "Connected" (green pill in the header)
...
- Refresh Data button triggers data reload (30s polling via React Query `refetchInterval`)
- Stale meters (>7 days without an update) appear in the "未更新のメーター" section
- Theme toggle switches light/dark, persists in `localStorage` under `theme`, and survives reload
...
- Frontend: Vite + React 18 + TypeScript + Tailwind CSS, TanStack Query for data fetching, Chart.js v4 for charts
- Frontend API base URL comes from `VITE_API_URL` (empty = same origin)
