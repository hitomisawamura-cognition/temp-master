...
The Dockerfile serves the Vite build from `switchbot-backend/static/`, but locally this directory doesn't exist. Build the frontend, then create a symlink:

```bash
cd switchbot-dashboard/switchbot-frontend
npm ci && npm run build
ln -s ../switchbot-frontend/dist ../switchbot-backend/static
```
...
- Footer: should say "Temp Master Dashboard v1.0 - Built with React + Bootstrap 3"
...
- Frontend: React 18 + TypeScript + Vite + Bootstrap 3 CSS + Chart.js 4
...
For frontend development, run `npm run dev` in `switchbot-frontend`. The Vite
development server runs on port 5173; set `VITE_API_URL` when using a different
backend URL.
