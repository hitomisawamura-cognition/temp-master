...
### 3. Build the frontend into the backend static directory

The Dockerfile builds `switchbot-frontend/` with Vite and copies `dist/` to `switchbot-backend/static/`.
Locally, either run the Vite dev server (`npm run dev`, http://localhost:5173) or build once and symlink the output:

```bash
cd switchbot-dashboard/switchbot-frontend && npm install && npm run build
...
- Theme toggle (sun/moon) switches dark mode and persists it in `localStorage`
...
- Frontend: React 18 + TypeScript + Vite + Tailwind CSS + Recharts (`VITE_API_URL` selects the backend)
