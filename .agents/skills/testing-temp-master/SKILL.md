...
For backend-served frontend testing, build the Vite app and symlink its
output as `static/`:
...
Alternatively, run the frontend development server separately:
...
- Connection status badge shows "Connected" with the green connected style

### UI Functionality
- Time Range selector: Last Hour / Last 24 Hours / Last 7 Days / Last 30 Days / Last Year
- Charts: Recharts responsive line charts rendered for active meters
- Refresh Data button triggers data reload
- Dark mode toggle switches themes and persists the selection
