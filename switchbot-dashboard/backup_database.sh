...
#   Set ADMIN_API_KEY environment variable to the backend's API key (required)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  ADMIN_API_KEY          API key sent as the X-API-Key header (required)"
            exit 0
            ;;
        *)
...
if [ -z "$ADMIN_API_KEY" ]; then
    echo "ADMIN_API_KEY is not set. The /api/backup endpoint requires an API key."
    exit 1
fi
...
backup_database() {
    ...
    local http_code=$(curl -s -w "%{http_code}" -o "$backup_file" -H "X-API-Key: $ADMIN_API_KEY" "$BACKEND_URL/api/backup")
