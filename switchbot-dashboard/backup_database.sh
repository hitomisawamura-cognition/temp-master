...
#   Set ADMIN_TOKEN environment variable (required; matches the backend ADMIN_TOKEN)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  ADMIN_TOKEN            Admin token required by /api/backup"
            exit 0
            ;;
        *)
...
if [ -z "$ADMIN_TOKEN" ]; then
    echo "ADMIN_TOKEN environment variable is required" >&2
    exit 1
fi
...
backup_database() {
    ...
    local http_code=$(curl -s -w "%{http_code}" -o "$backup_file" \
        -H "Authorization: Bearer $ADMIN_TOKEN" "$BACKEND_URL/api/backup")
